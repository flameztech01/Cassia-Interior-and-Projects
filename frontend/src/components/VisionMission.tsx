...
const VisionMission = () => {
  ...
  return (
    ...
    >
      {/* Sophisticated background texture - fades in */}
      <div 
      ...
      <div 
        className={`absolute top-40 left-20 w-72 h-72 border border-[#2F7D76]/20 rounded-full animate-pulse transition-all duration-1500 delay-400 ${
          isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-50'
        }`} 
      />
      <div 
        className={`absolute bottom-40 right-20 w-96 h-96 border border-[#E6E08A]/10 rounded-full animate-pulse delay-1000 transition-all duration-1500 delay-600 ${
          isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-50'
        }`} 
      />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="group relative">
            ...
            <div 
              className={`relative bg-white/5 backdrop-blur-sm rounded-3xl p-12 border border-white/10 hover:border-[#2F7D76]/30 transition-all duration-1000 delay-800 transform ${
            ...
            >
              ...
              <div className="mt-12 space-y-8">
                {[
                  ...
                  <p 
                    key={index}
                    className={`text-xl text-white/70 leading-relaxed font-light transition-all duration-1000 transform ${
                      isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
                    }`}
        ...
        <div 
          className={`absolute -bottom-32 -right-32 w-96 h-96 bg-[#2F7D76]/10 rounded-full blur-3xl -z-10 transition-opacity duration-1500 delay-1400 ${
            isVisible ? 'opacity-100' : 'opacity-0'
          }`} 
        />
        <div 
          className={`absolute -top-32 -left-32 w-96 h-96 bg-[#E6E08A]/10 rounded-full blur-3xl -z-10 transition-opacity duration-1500 delay-1400 ${
            isVisible ? 'opacity-100' : 'opacity-0'
          }`} 
        />
          ...
          {[0, 1, 2, 3].map((index) => (
            <div 
              ...
              className={`w-2 h-2 rounded-full transition-all duration-1000 ${
                index % 2 === 0 ? 'bg-[#2F7D76]/30' : 'bg-[#E6E08A]/30'
      ...
      <div 
        className={`absolute bottom-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-[#2F7D76]/40 to-transparent transition-all duration-1500 delay-1800 ${
          isVisible ? 'opacity-100' : 'opacity-0'
        }`} 
      />
    </section>
